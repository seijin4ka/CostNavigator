...
`.trim();
  ...
  try {
    ...
    const targetKv = namespaces.find(kv => kv.title === KV_NAMESPACE);

    if (targetKv) {
      kvId = targetKv.id;
      console.log(`✅ 既存のKV名前空間を使用: ${kvId}\n`);
    }
  } catch (error) {
    console.log('   KV名前空間一覧取得でエラー（新規作成を試みます）');
  }

  if (!kvId) {
    console.log('🗄️ KV名前空間を新規作成中...');
    try {
      const createOutput = execSync(`npx wrangler kv:namespace create --title="${KV_NAMESPACE}" --json`, {
        encoding: 'utf-8',
        stdio: ['pipe', 'pipe', 'pipe']
      });
      ...
      if (result && result.id) {
        kvId = result.id;
        console.log(`✅ KV名前空間を作成しました: ${kvId}\n`);
      }
    } catch (error) {
      const errorMessage = error.message || error.toString();
      if (errorMessage.includes('already exists')) {
        console.log('   KV名前空間は既に存在します。再度リストから取得を試みます...');
        try {
          const listOutput = execSync('npx wrangler kv:namespace list --json', {
            encoding: 'utf-8',
            stdio: ['pipe', 'pipe', 'pipe']
          ...
          console.error('   KV名前空間ID取得に失敗しました');
        }
      } else {
        console.error('❌ KV名前空間の作成に失敗しました');
        console.error(error.message);
      }
    }
  }

  // KV名前空間IDが取得できた場合のみwrangler.jsonを更新
  if (kvId && fs.existsSync(WRANGLER_JSON_PATH)) {
    console.log('📝 KV名前空間を設定ファイルに追加中...');
    const wranglerConfig = JSON.parse(fs.readFileSync(WRANGLER_JSON_PATH, 'utf-8'));

    // KVバインディングを更新
    if (wranglerConfig.kv_namespaces && wranglerConfig.kv_namespaces.length > 0) {
      wranglerConfig.kv_namespaces[0].id = kvId;
      fs.writeFileSync(WRANGLER_JSON_PATH, JSON.stringify(wranglerConfig, null, 2));
      console.log('✅ KV名前空間を設定ファイルに追加しました\n');
    } else {
      console.error('❌ wrangler.jsonにkv_namespacesが見つかりません');
    }
  }
